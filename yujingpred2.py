...
from dbutil.db2df import get_k_data, get_suspend_df
from util import tunshare as tn
from util import util
from yuyingpred1 import read_result, get_calender
...
def get_trade_strategy():
    trade_strategy = pd.Series(index=['buy', 'sell', 'longshort'], data=['open', 'open', 'long'])
...
def check_trade_date(ts_code, trade_date):
    ...
    if trade_date >= delist_date:
        return False
    suspend = get_suspend_df(ts_code, trade_date)
    if suspend is None:
        return False
    return True
...
def extract_factors(ts_code, start, end, ndate):
    ...
    industry_df = stock_info[(stock_info['ts_code'] == ts_code)]
    if len(industry_df) == 0:
        print('industry_df is None', ts_code)
        industry = 1000
    else:
        industry = industry_df.industry_code.values[0]
...
if __name__ == '__main__':
    ...
    for pos in range(positions, positions + 5, 5):
        ...
        if max < result[-1:].sum_pure_return.values[0]:
            ...
            print('最大回撤:', MaxDrawdown(total_net_date_value))
        ...
        title = title + ' ' + 'maxdrawn:' + str(MaxDrawdown(total_net_date_value)) + '\n'
